...
Vial の **Combos** タブで **128 個**まで設定できる(RMK の既定は 8)。
1コンボにつき最大4キーの組み合わせ → 出力1キー。

- 個数は [`keyboard.toml`](keyboard.toml) の `[rmk] combo_max_num` で決まる(上限 256)。
  rmk クレートの build.rs がこれをコンパイル時定数にするので、変更には再ビルドが必要。
  ファイルの場所は `.cargo/config.toml` の `[env] KEYBOARD_TOML_PATH` で渡している。
- 保存領域は `src/main.rs` の `StorageConfig.num_sectors`(32KB)で確保している。
  コンボ数をさらに増やすときはここも合わせて見直すこと。
- RMK のコンボ判定は**同時押し(押し重ね)**。キーを離してから次を押す順次入力では
  発動しない(判定時間の既定は 50ms)。
